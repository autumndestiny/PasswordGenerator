...
function randomIndex(str){
  ...
  function generatePassword(upper, lower, number, symbol, length){
    ...
    // Removing extra characters if necessary (The above loop will create a passwrod that may not match the length selected if that length is not a multiple of the number of types selected.)
...
//   Event listener for when teh "Generatee Password" button is clicked
generateEl.addEventListener(`click`, () => {
    // Changing a value from a string to a number
   const length = parseInt(lengthEl.nodeValue);

//    Checking if the follwoing options/checkboxes are selected and setting the returned true/false values to the respective variables.
